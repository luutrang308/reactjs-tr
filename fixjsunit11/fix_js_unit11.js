...
function checkPassFailStudent (students){
  return students.reduce(function(result, student){
	var nameStudentSplit = student.name.split(' ');
	switch(true){
...
function getStudentWithCustomScore (students){
  return students.filter(student => {
	var score 					= (student.score + '').split('.');
	var sumScore 				= parseInt(score[0]) + parseInt(score[1]);
	var splitSumScore 			= (sumScore + '').split('');
	var lastCharacterSumScore 	= splitSumScore.pop();
